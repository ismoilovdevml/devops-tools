...
def get_jobs():
    response = requests.get(f'{jenkins_url}/api/json?tree=jobs[name,lastBuild[result,timestamp]]')
...
def get_branches(job_name):
    response = requests.get(f'{jenkins_url}/job/{job_name}/api/json?tree=jobs[name,lastBuild[result,timestamp]]')
    ...
    if response.status_code == 200:
        ...
        if 'jobs' in data:
            for job in data['jobs']:
                ...
                if branch_name.lower().startswith(('dev', 'stage', 'prod')):
